...
def test_boot_fixture(entry, rom, sym, tmp_path, artifacts_dir):
    ...
    try:
        ...
        # Read WRAM right at the EnterMap hook: LoadSAV already ran
        # (CONTINUE), and no further frame has ticked, so the values are
        # exactly what the engine loaded from the save.
...
def test_overworld_survives_after_continue(entry, rom, sym, tmp_path, artifacts_dir):
    """The overworld keeps running after CONTINUE (LCD on, play-time clock
    ticking). This guards the cached map-header/engine-state block that
    new_empty bakes in (gen1/engine_state.rs): the engine trusts that block
    from the save on CONTINUE (LoadSAV sets BIT_NO_PREVIOUS_MAP, so
    LoadMapHeader never reloads it from ROM), and a zeroed block used to
    crash the game (rst $38 wild jump) about one second in.
    """
    name = entry["file"]
    rom_copy = tmp_path / "pokered.gbc"
    sav_copy = tmp_path / name
